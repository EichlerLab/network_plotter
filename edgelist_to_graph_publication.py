...
#Date: 08/12/2016
#Updated on 10/22/2019 by Tychele N. Turner, Ph.D. to create publication quality pdf files 
...
#Example:
#python edgelist_to_graph_publication.py full_mod_list.txt edgelist_all.txt edgelist_within_mods.txt RandomGeneList.1.txt clean_layout.txt True [mod1,mod2,mod4]
...
def getSize(node, default_size= 300.0):
    if "MutationScore" in node:
        return default_size + ((node["MutationScore"])*500)
    else:
        return default_size
...
def getColor(node):
    if node["MutationScore"] == 0.0:
        return (0.19,0.21,0.58)
    elif node["missense"]== 0.0 and node["LoF"]== 0.0:
    	return (1,1,1)
    elif node["MutationScore"] != 0.0:
        if round(float(node["LoF"])/(float(node["LoF"])+float(node["missense"])), 1) == 1.0:
            return(0.65,0,0.15)
        if round(float(node["LoF"])/(float(node["LoF"])+float(node["missense"])), 1) == 0.9:
            return(0.84,0.19,0.15)
        if round(float(node["LoF"])/(float(node["LoF"])+float(node["missense"])), 1) == 0.8:
            return(0.96,0.43,0.26)
        if round(float(node["LoF"])/(float(node["LoF"])+float(node["missense"])), 1) == 0.7:
            return(0.99,0.68,0.38)
        if round(float(node["LoF"])/(float(node["LoF"])+float(node["missense"])), 1) == 0.6:
            return(1,0.88,0.56)
        if round(float(node["LoF"])/(float(node["LoF"])+float(node["missense"])), 1) == 0.5:
            return(1,1,0.6)
        if round(float(node["LoF"])/(float(node["LoF"])+float(node["missense"])), 1) == 0.4:
            return(0.88,0.95,0.97)
        if round(float(node["LoF"])/(float(node["LoF"])+float(node["missense"])), 1) == 0.3:
            return(0.67,0.85,0.91)
        if round(float(node["LoF"])/(float(node["LoF"])+float(node["missense"])), 1) == 0.2:
            return(0.45,0.68,0.82)
        if round(float(node["LoF"])/(float(node["LoF"])+float(node["missense"])), 1) == 0.1:
            return(0.27,0.46,0.71)
    else:
        return (0,0,0)
...
nx.draw_networkx(finalgraph, ax=ax, pos=layout, node_size=nodesize, node_color=nodecolor, style=style, edge_color=colors, width=widths, font_size=26, font_weight="bold")

plt.savefig("graphfigure.pdf", transparent=True, bbox_inches="tight", format="pdf")

while True:
	
	print "'graphfigure.pdf' has been saved"
